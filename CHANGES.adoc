...
- Run the test suite against SQLite instead of H2, dropping the H2 dependency in
  favour of `org.xerial/sqlite-jdbc`. No library code changed: `src/` is identical
  to 0.9.2, and this only affects what the tests run against.
- Cover `repeatable-read` in the isolation-level test, which H2 could not express
  because it upgraded that level to serializable.
